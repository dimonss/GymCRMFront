...
Приложение будет доступно на `http://localhost:5175/` в режиме разработки.

## Продакшен деплой

Приложение настроено для размещения по пути `/gym_crm/` на продакшене.

**Конфигурация:**
- `vite.config.ts` - настроен `base: '/gym_crm/'` для продакшена
- `App.tsx` - использует `basename` из `import.meta.env.BASE_URL`
...
Собранные файлы будут в папке `dist/`. Разместите содержимое этой папки в директории `/gym_crm/` на вашем сервере.

**Важно:** Все роуты будут работать с префиксом `/gym_crm/`:
- `/gym_crm/` - главная страница
- `/gym_crm/application` - форма заявки
- `/gym_crm/crm` - CRM dashboard
- `/gym_crm/admin` - админ панель
